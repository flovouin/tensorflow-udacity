# Provides utility functions when training neural networks.
...
def gaussian_weights_variable(shape):
    """Initialises a weight variable with a truncated Gaussian distribution."""
...
def uniform_weights_variable(shape):
    """Initialises a weight variable with a uniform distribution."""
    max_abs = 1.0 / np.sqrt(shape[0])
    return tf.Variable(tf.random_uniform(shape, -max_abs, max_abs))

def gaussian_kernels_variable(shape):
    """Initialises a variable containing kernels for a CNN
    with a truncated Gaussian distribution."""
...
def logprob(predictions, labels):
    """Returns the log-probability of the true labels in a predicted batch."""
    predictions[predictions < 1e-10] = 1e-10
    return np.sum(np.multiply(labels, -np.log(predictions))) / labels.shape[0]

def sample_distribution(distribution):
    """Samples one element from a distribution assumed to be an array of normalized
    probabilities."""
...
def sample(prediction):
    """Turns a (column) prediction into 1-hot encoded samples."""
